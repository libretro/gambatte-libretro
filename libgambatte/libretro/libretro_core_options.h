...
struct retro_core_option_v2_definition option_defs_us[] = {
    {"gambatte_gb_colorization",
     "GB Colorization",
     NULL,
     "Enables colorization of Game Boy games. 'Auto' selects the 'best' (most "
     "colorful/appropriate) palette. 'GBC' selects game-specific Game Boy "
     "Color palette if defined, otherwise 'GBC - Dark Green'. 'SGB' selects "
     "game-specific Super Game Boy palette if defined, otherwise 'SGB - 1A'. "
     "'Internal' uses 'Internal Palette' core option. 'Custom' loads "
     "user-created palette from system directory.",
     NULL,
     NULL,
     {
         {"disabled", NULL},
    ...
    {"gambatte_gb_internal_palette",
     "Internal Palette",
     NULL,
     "Selects palette used for colorizing Game Boy games when 'GB "
     "Colorization' is set to 'Internal', or when 'GB Colorization' is set to "
     "'Auto' and game has no pre-defined SGB/GBC palette. 'GB' palettes mimic "
     "the display of original Game Boy hardware. 'GBC' palettes are identical "
     "to the built-in presets of the Game Boy Color. 'SGB' palettes are "
     "identical to the built-in presets of the Super Game Boy.",
     NULL,
     NULL,
     {
         {"GB - DMG", NULL},
    ...
    {"gambatte_gb_palette_twb64_1",
     ...
     NULL,
     "Selects internal colorization palette when 'Internal Palette' is set to "
     "'TWB64 - Pack 1'.",
     NULL,
     NULL,
     {
    ...
    {"gambatte_gbc_color_correction",
     "Color Correction",
     NULL,
     "Adjusts output colors to match the display of real Game Boy Color "
     "hardware. 'GBC Only' ensures that correction will only be applied when "
     "playing Game Boy Color games, or when using a Game Boy Color palette to "
     "colorize a Game Boy game. 'Always' applies color correction to all "
     "content, and will produce unexpected/suboptimal results when using 'GB' "
    ...
    {"gambatte_gbc_color_correction_mode",
     ...
     NULL,
     "Specifies method used when performing color correction. 'Accurate' "
     "produces output almost indistinguishable from a real Game Boy Color LCD "
     "panel. 'Fast' merely darkens colors and reduces saturation, and may be "
     "used on low-end hardware if the 'Accurate' method is too slow.",
     NULL,
     NULL,
     {
         {"accurate", "Accurate"},
    ...
    {"gambatte_gbc_frontlight_position",
     ...
     NULL,
     "Simulates the physical response of the Game Boy Color LCD panel when "
     "illuminated from different angles. 'Central' represents standard color "
     "reproduction. 'Above Screen' increases brightness. 'Below Screen' "
     "reduces brightness. This setting only applies when 'Color Correction "
     "Mode' is set to 'Accurate'.",
     NULL,
     NULL,
     {
         {"central", "Central"},
    ...
    {"gambatte_dark_filter_level",
     "Dark Filter Level (%)",
     NULL,
     "Enable luminosity-based brightness reduction. May be used to avoid "
     "glare/eye strain when playing games with white backgrounds, which are "
     "intended for display on a non-backlit Game Boy Color and can therefore "
     "appear uncomfortably bright when viewed on a modern backlit screen.",
     NULL,
     NULL,
     {
         {"0", NULL},
    ...
    {"gambatte_mix_frames",
     "Interframe Blending",
     NULL,
     "Simulates LCD ghosting effects. 'Simple' performs a 50:50 mix of the "
     "current and previous frames. 'LCD Ghosting' mimics natural LCD response "
     "times by combining multiple buffered frames. 'Simple' blending is "
     "required when playing games that rely on LCD ghosting for transparency "
     "effects (Wave Race, Ballistic, Chikyuu Kaihou Gun ZAS...).",
     NULL,
     NULL,
     {
         {"disabled", NULL},
    ...
    {"gambatte_audio_resampler",
     "Audio Resampler",
     NULL,
     "Specify which algorithm to use when resampling generated audio (the Game "
     "Boy audio rate is limited only by its CPU write speed, such that "
     "'native' frequencies are impractical on modern sound devices and must be "
     "downsampled). 'Sinc' produces the highest quality. 'Cosine' improves "
...
#if (defined(PS2) || defined(PSP) || defined(VITA) || defined(_3DS) ||         \
     defined(DINGUX))
     "cc"
#else
      "sinc"
    ...
    {"gambatte_gb_hwmode",
     ...
     NULL,
     "Specify which type of hardware to emulate. 'Auto' is recommended. "
     "Selecting 'GBA' unlocks extra features in certain 'GBA Enhanced' Game "
     "Boy Color games (Shantae, Wendy - Every Witch Way, Legend of Zelda: "
    ...
    {"gambatte_gb_bootloader",
     ...
     NULL,
     "Enable support for official Game Boy and Game Boy Color bootloaders, "
     "with corresponding start-up logo animations.",
     NULL,
     NULL,
     {
         {"enabled", NULL},
    ...
    {"gambatte_up_down_allowed",
     ...
     NULL,
     "Enabling this will allow pressing/quickly alternating/holding both left "
     "and right (or up and down) directions at the same time. This may cause "
     "movement-based glitches.",
     NULL,
     NULL,
     {
         {"disabled", NULL},
    ...
    {"gambatte_turbo_period",
     "Turbo Button Period",
     NULL,
     "Specify the repeat interval (in frames) when holding down the Turbo A/B "
     "buttons.",
     NULL,
     NULL,
     {
    ...
    {"gambatte_rumble_level",
     ...
     NULL,
     "Enables haptic feedback effects for supported games (Pokemon Pinball, "
     "Perfect Dark...).",
     NULL,
     NULL,
     {
...
struct retro_core_options_v2 options_us = {NULL, option_defs_us};
  ...
  *categories_supported = false;
  ...
  if (version >= 2) {
    ...
    *categories_supported = environ_cb(
        RETRO_ENVIRONMENT_SET_CORE_OPTIONS_V2_INTL, &core_options_intl);
#else
    *categories_supported =
    ...
    struct retro_core_option_v2_definition *option_defs_intl = NULL;
    struct retro_core_option_definition *option_v1_defs_intl = NULL;
    ...
    if (version >= 1) {
      /* Allocate US array */
      option_v1_defs_us = (struct retro_core_option_definition *)calloc(
          num_options + 1, sizeof(struct retro_core_option_definition));

      /* Copy parameters from option_defs_us array */
      for (i = 0; i < num_options; i++) {
        struct retro_core_option_v2_definition *option_def_us =
            &option_defs_us[i];
        struct retro_core_option_value *option_values = option_def_us->values;
        struct retro_core_option_definition *option_v1_def_us =
            &option_v1_defs_us[i];
        struct retro_core_option_value *option_v1_values =
            option_v1_def_us->values;
      ...
      if (option_defs_intl) {
        ...
        /* Allocate intl array */
        option_v1_defs_intl = (struct retro_core_option_definition *)calloc(
            num_options_intl + 1, sizeof(struct retro_core_option_definition));

        /* Copy parameters from option_defs_intl array */
        for (i = 0; i < num_options_intl; i++) {
          struct retro_core_option_v2_definition *option_def_intl =
              &option_defs_intl[i];
          struct retro_core_option_value *option_values =
              option_def_intl->values;
          struct retro_core_option_definition *option_v1_def_intl =
              &option_v1_defs_intl[i];
          struct retro_core_option_value *option_v1_values =
              option_v1_def_intl->values;
      ...
      environ_cb(RETRO_ENVIRONMENT_SET_CORE_OPTIONS_INTL,
      ...
      variables = (struct retro_variable *)calloc(
          num_options + 1, sizeof(struct retro_variable));

...
namespace gambatte
{
   ...
   {
      // original values are 5 bits, want to scale up to 8 bit values.
      const unsigned r = (bgr15       & 0x1F) << 3;
      const unsigned g = (bgr15 >>  5 & 0x1F) << 3;
      const unsigned b = (bgr15 >> 10 & 0x1F) << 3;
...
#else
      return (0xff<<24) | (rFinal<<0) | (gFinal << 8) | (bFinal<<16);
#endif
   }
